...
The NVIDIA, Gemini, Groq, OpenRouter, Anthropic, MuAPI, ElevenLabs, HeyGen, and NVIDIA Image Generation integrations used by Creator Studio are server-side only. Configure their credentials as deployment environment variables. The preserved direct OpenAI and Runway adapters are deferred and are not reachable through the active private Creator Studio dispatch or UI. Protect `/api/creator/*` with a separate GitHub OAuth application, `CREATOR_SESSION_SECRET`, and the `CREATOR_GITHUB_ALLOWED_USER_IDS` and/or `CREATOR_GITHUB_ALLOWED_LOGINS` allowlist.

The Selena Brain Router separates reasoning from generation. `NVIDIA_API_KEY`, `GEMINI_API_KEY`, `GROQ_API_KEY`, `OPENROUTER_API_KEY`, and `ANTHROPIC_API_KEY` are read only inside the server adapters and are never placed in `NEXT_PUBLIC_*`, browser storage, URLs, response bodies, or provider-status payloads. API keys are sent only as authorization headers to fixed official provider hosts. Model names and routing order are normal configuration values, not credentials. `NVIDIA_API_KEY` is also shared by the separate NVIDIA Image Generation provider (`src/lib/nvidiaCreatorProvider.js`), which keeps its own `NVIDIA_IMAGE_MODEL` configuration distinct from the Brain's `NVIDIA_BRAIN_MODEL` and resolves it only against a fixed server-side model registry -- the browser can never select an arbitrary model or upstream host.

Fallback is bounded by `BRAIN_MAX_ATTEMPTS` (default: the full configured chain length) and is allowed for transient availability/capacity failures, provider timeouts, malformed responses, unsupported required capabilities, and a provider that is simply unconfigured (missing its API key) when automatic fallback is enabled -- a misconfigured primary provider is skipped rather than stopping the chain. Invalid input, safety rejection, an explicit `providerOverride`'s missing credentials, and a missing key when automatic fallback is disabled all stop immediately instead of falling back. Requests classified as paid generation, publishing, another external mutation, or requiring explicit approval also disable fallback. The router returns tool calls but does not execute them, preventing a reasoning retry from duplicating an external side effect.
...
This keyword check operates on JSON *text* fields (prompt/message/instruction/description/caption/text/query/content/script/input/system/task) -- it is not image-content moderation and must not be treated as one. NVIDIA Image Generation's optional base64 `referenceImage` field is not a text field, so its image bytes are never scanned by this check. That reference-image path still enforces every other existing boundary (GitHub-authenticated session, same-origin mutation check, per-identity rate limit, and provider-side base64/MIME-type/size validation -- see `docs/CREATOR_STUDIO.md`), but there is currently no image-content moderation capability in this repository. Document that gap rather than presenting keyword matching as image moderation; a deployment that accepts untrusted reference images should add a dedicated image-content moderation service in front of that route.
