...
The chain is also bounded in wall-clock time, independently of attempt count. Five providers at up to 45 seconds each is technically possible (roughly 225 seconds worst case) but operationally undesirable inside a bounded serverless function, so `BRAIN_MAX_TOTAL_MS` (default `90000`, i.e. 90s) caps the whole fallback chain's total execution time. Once starting another provider could not realistically finish inside the remaining budget, the router stops rather than beginning an attempt the platform would simply cut off; it still returns the most recent provider's error rather than exhausting the full configured order. This budget is sized to fit comfortably inside the Creator API route's own explicit `maxDuration` (150s -- see "Execution budget" below), which itself no longer depends on the platform's default function timeout.
...
### Migration notes: Brain Router defaults changed

An earlier revision of this router defaulted to Gemini as the primary brain, fell back through `groq,openrouter` only, and capped `BRAIN_MAX_ATTEMPTS` at `3`. This revision intentionally changes all three defaults:

- **Default provider**: `gemini` -> `nvidia`. `BRAIN_PROVIDER` now defaults to `nvidia` when unset.
- **Fallback chain**: now includes Anthropic. `BRAIN_FALLBACK_ORDER` defaults to `gemini,groq,openrouter,anthropic` (four fallbacks behind NVIDIA, five providers total in the chain).
- **`BRAIN_MAX_ATTEMPTS`**: `3` -> the configured chain's full length (5 by default) rather than a fixed number, so the default no longer silently stops short of the last configured fallback.

A deployment that wants the previous behavior (Gemini primary, no Anthropic fallback, at most 3 attempts) must configure that explicitly rather than relying on defaults: set `BRAIN_PROVIDER=gemini`, `BRAIN_FALLBACK_ORDER=groq,openrouter`, and `BRAIN_MAX_ATTEMPTS=3` in that environment. Existing environments that already set these variables explicitly are unaffected; only environments relying on the previous unset/default values will see different routing after this change.

### Execution budget

`app/api/creator/[[...path]]/route.js` (which serves the Brain Assistant, NVIDIA Image Generation, and every other Creator Studio provider route) sets an explicit `export const maxDuration = 150` rather than depending on the platform's default function timeout. That budget comfortably covers the slowest single upstream call already made on this route (ElevenLabs speech, up to 120s) and NVIDIA Image Generation (up to ~60s), plus the Brain Router's own bounded `BRAIN_MAX_TOTAL_MS` fallback budget (default 90s, see above) and request/response overhead.
...
`NVIDIA_IMAGE_MODEL` is resolved against a fixed server-side registry (currently [`black-forest-labs/flux.2-klein-4b`](https://build.nvidia.com/black-forest-labs/flux.2-klein-4b)). An unrecognized or unset value falls back to the reviewed default instead of being forwarded upstream, and the browser can never select a model or upstream URL directly. Requests accept `prompt`, an optional base64 `referenceImage` (for editing), `aspectRatio`, `seed`, and `steps`; the normalized result matches the existing binary-image provider contract already used by the deferred OpenAI adapter.

**Status: implemented, not live-provider verified.** Both capabilities are implemented against NVIDIA's documented hosted contract, but neither has been exercised against a live, billed NVIDIA account -- `nvidiaImageProviderStatus()` reports `tested: false` and `productionReady: false` for this reason. The two capabilities are not equally mature:

- **Generation** (no `referenceImage`) follows the documented request/response shape directly and is the better-understood path.
- **Editing** (`referenceImage` supplied) is **experimental and requires live verification**. The documented hosted contract only fully specifies the generation payload; the editing-mode `image` field is a best-effort carryover pending a live hosted-account check (see the code comment in `normalizeNvidiaImageRequest`). The provider-status payload exposes this distinction as `capabilityStatus: { generate: 'implemented', edit: 'experimental-requires-live-verification' }` -- do not present editing to users as unconditionally supported, and do not claim `productionReady` for either capability until it has been verified live.

A `referenceImage` still passes the same authentication, same-origin, rate-limit, and size/type validation as every other Creator Studio generation request (see "Security model" below), and its request body is accepted up to a larger, dedicated limit (`MAX_NVIDIA_IMAGE_JSON_BODY_BYTES`, 16 MiB) sized for the provider's own 8 MiB reference-image cap -- the general 64 KiB Creator JSON body limit used by other routes would otherwise reject every real edit request outright. There is currently no image-content moderation for `referenceImage` bytes: the built-in content-safety check (`CONTENT_SAFETY_MODE`) only inspects text-shaped JSON fields (prompt/message/instruction/etc.) for keyword matches, and a base64 image blob under the `referenceImage` key is never scanned as text. Treat that as a documented gap, not a moderation guarantee -- a production deployment that accepts untrusted reference images should add a dedicated image-content moderation service in front of this route.
...
- NVIDIA Image Generation's `referenceImage` upload path enforces the same GitHub-authenticated session, same-origin mutation check, per-identity rate limit, and provider-side base64/MIME/size validation as every other generation route (see "Configure NVIDIA Image Generation" above). It has no image-content moderation: the built-in content-safety check is text-keyword matching over JSON fields, not image analysis, and it never inspects reference-image bytes.
